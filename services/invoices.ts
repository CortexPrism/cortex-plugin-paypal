  ...
  execute: async (args: Record<string, unknown>, ctx: PluginContext): Promise<ToolResult> => {
    ...
    try {
      ...
      if (!createResponse.ok) {
        return await handleResponse('paypal_create_invoice', createResponse, start);
      }
      ...
      if (sendInvoice && invoice.id) {
        ...
        if (!sendResponse.ok) {
          return await handleResponse('paypal_create_invoice', sendResponse, start);
        }
      }
...
export const paypalShowInvoiceTool: Tool = {
  definition: {
    ...
    params: [
      { name: 'invoice_id', type: 'string', description: 'PayPal invoice ID', required: true },
    ],
    capabilities: ['network:fetch'],
  },
  execute: async (args: Record<string, unknown>, ctx: PluginContext): Promise<ToolResult> => {
